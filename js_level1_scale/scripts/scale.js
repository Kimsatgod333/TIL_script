...
//결과예) 당신의 적정체중은 ?Kg이며 ?Kg 초과 되셨습니다.
//미만, 적정체중은 조건문을 배워야 가능하기 때문에 나중에 자바스크립트 추가예정
...
//값을 실시간으로 입력하는 동안은 프로그램 과부하 및 보안 문제로 함수를 작동하게 해서는 안되며 결정버튼을 따로 만들어두고 버튼에 특정동작을 해야만 함수가 실행되게 만들어야 한다.
// 살사건 함수 허용하는 경우 : 보안이 필요하지 않은 가벼운 프로그래밍 부분 예: 리뷰 글자수 제한등)
...
function weightCal(){
    ...
    console.log(`당신의 적정체중은${normalWeight}Kg이며 ${userWeight.value - normalWeight}Kg 초과 되셨습니다.`);
